...
void gravar_jogo(struct BaseTable *tabuleiro, struct BaseTable *tiros, int pl){    
    //Usaremos estes vetores para salvar os elementos dos tabuleiros
    char tabuleiro_P1[256], tabuleiro_P2[256];
    char tiros_P1[256], tiros_P2[256];
        
    //Este laço associa as posições das matrizes aos respectivos vetores
    int k = 0;
    for (int i = 0; i<16; i++){
        for (int j = 0; j<16;j++){
        tabuleiro_P1[k] = tabuleiro[0].posicao[i][j];
        tabuleiro_P2[k] = tabuleiro[1].posicao[i][j];
       
        tiros_P1[k] = tiros[0].posicao[i][j];
        tiros_P2[k] = tiros[1].posicao[i][j];
   ...
   //Criamos o arquivo de texto onde iremos gravar o progresso do jogo e o abrimos para escrita
    ...
    if (game_save == NULL){
        printf ("\n\t Ocorreu um erro desonhecido ao salvar o arquivo\n");
    }
    ...
    fprintf (game_save, "%d;%d;", tiros[0].N, tiros[1].N);              // casas livres no tabuleiro e vez do jogador)
    fprintf (game_save, "%d;%d;%d;", tabuleiro[0].N, tabuleiro[1].N, pl);
