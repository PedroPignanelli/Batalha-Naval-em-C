...
for (int player = 0; player < 2; player++){
    if (player==0) printf ("\nAs aeronaves de patrulha avistaram a armada inimiga...");
...
void gravar_jogo(struct BaseTable *tabuleiro, struct BaseTable *tiros){
    ...
    char file_name[40];
    sprintf(file_name, "tabuleiro_%d-%d-%d-%d.txt", time_s->tm_yday, time_s->tm_hour, time_s->tm_min, time_s->tm_sec); 
   
   //Criamos o arquivo de texto onde iremos gravar o progresso do jogo e o abrimos para escrita
    FILE *game_save;
    game_save = fopen(file_name,"w");
    if (game_save == NULL){
    ...
    fprintf (game_save, ",");
    fprintf (game_save, "\n%f\n%f", tiros[0].pontos, tiros[1].pontos);    // Salvamos os outros dados do jogo (pontuação, tiros dados,
    fprintf (game_save, "\n%d\n%d", tiros[0].N, tiros[1].N);              // casas livres no tabuleiro e vez do jogador)
    fprintf (game_save, "\n%d\n%d ", tabuleiro[0].N, tabuleiro[1].N);
...
void carregar_jogo(struct BaseTable *tabuleiro, struct BaseTable *tiros){
    ...
    if (game_load == NULL){
        printf ("\nNao foi possivel abrir o arquivo. Fechando janela");
        fclose (game_load);
        exit (0);
    }
    else{
        printf ("Opened");
        char RtabuleiroP1[257], RtabuleiroP2[257], RtirosP1[257], RtirosP2[257];
        char pts2[10];
        char p[5];
        fscanf (game_load, "%s[^,]%s[^,]%s[^,]%s[^,]%s%s", RtabuleiroP1, RtabuleiroP2, RtirosP1, RtirosP2, pts2, p);    
       
        printf ("\n%s", p);
        printf ("\n%s", pts2);

        // tiros[0].pontos , tiros[1].pontos, tiros[0].N, tiros[1].N, tabuleiro[0].N, tabuleiro[1].N
