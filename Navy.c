...
int marcar_tiro(struct BaseTable *tiros, struct BaseTable *tabuleiro, struct Remainings *contador, int jogador, int lin_temp, char col_temp);
...
int carregar_jogo(struct BaseTable *tabuleiro, struct BaseTable *tiros, int pl);
...
int main(){
    ...
    while (flagmain == 0){
        ...
        switch (comand_menu){
            ...
            case 2: 
                pl = carregar_jogo(tabuleiro, tiros, pl);   //Carrega algum jogo salvo
                flagmain = 1;
            break;
    ...
    while (status == 0){       //Loop para funcionamento do jogo, que só será rompido caso solicitado pelo jogador
        ...
        if (strcmp (comando, "SAIR") == 0){
            ...
            if (confirm == 1){
                ...
                return 0;
            }else {confirm = 0;}   //redefine o valor de confirm
            ...
            hit = marcar_tiro (tiros, tabuleiro, contador, jogador, lin_temp, col_temp);
            //Soma 1 ao valor de pl se o jogador errou o tiro, "alternando" quem está atirando
            if (hit == 0){
            pl = pl+1;
            }
        }
            ...
            gravar_jogo(tabuleiro, tiros, jogador);
        }
...
int marcar_tiro(struct BaseTable *tiros, struct BaseTable *tabuleiro, struct Remainings *contador, int jogador, int lin_temp, char col_temp){  //Função que compara as matrizes e define se o jogador conseguiu um acerto ou não
    int hit;
...
return hit;// usamos o parametro 'hit' para determinar se o jogador acertou e, assim, atirar denovo
}
...
void gravar_jogo(struct BaseTable *tabuleiro, struct BaseTable *tiros, int pl){
   ...
   //Criamos o arquivo de texto onde iremos gravar o progresso do jogo e o abrimos para escrita
    ...
    fprintf (game_save, "%d;", pl);

    fclose (game_save);//fecha o arquivo e gera um feedback na tela.
    printf ("\n\tJogo salvo com sucesso!\n");
}

int carregar_jogo(struct BaseTable *tabuleiro, struct BaseTable *tiros, int pl){    //Definida com int para retornarmos de quem é a vez
    ...
    if (game_load == NULL){   
        ...
        //Esses comandos são para lerem os valores salvos no arquivo, a fim de inserí-los no jogo
        ...
        fscanf (game_load, "%f;%f;%d;%d;%d;%d;%d", &tiros[0].pontos, &tiros[1].pontos, &tiros[0].N, &tiros[1].N, &tabuleiro[0].N, &tabuleiro[1].N, &pl);

        //Assimilando os valores às referentes variáveis
